...
class HashTable:
    ...
    def add(self, key, value):
        ...
        Take two arguments representing a key-value pair, and compute the hash 
        of the key.
        ...
        their key-value pairs should be stored in the existing nested 
        dictionary under the same hash value.
        """
        hashed_key = self.hash(key)
        if hashed_key in self.collection:
            synonym = self.collection[hashed_key] 
            synonym |= {key: value}
    ...
    def remove(self, key):
        ...
        it should not raise an error or remove anything.
        """
        hashed_key = self.hash(key)
        if hashed_key in self.collection:
    ...
    def lookup(self, key):
        ...
        """
        hashed_key = self.hash(key)
        return self.collection.get(hashed_key, {}).get(key, None)

test = HashTable()
print(test.hash(" "))
test.add("niche", "Petit Habitat de jardin réservé au animaux de compagnie")
test.add("chien", "De la famille des canin lipus")
