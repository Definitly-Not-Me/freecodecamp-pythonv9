...
class Player(abc.ABC):
    ...
    def make_move(self):
        """
         The Player class should have a method named make_move that:
        Uses random.choice to get a random move from 
            the moves attribute (defined in the concrete class).
        Adds the values from the selected move to the current position 
            and updates the position attribute.
        Appends the new position tuple to the path attribute.
        Returns the new position.
        """
        mv :tuple = random.choice(self.moves)
        new_position = (mv[0] + self.position[0], mv[1] + self.position[1])
    ...
    def level_up(self):
        """
        The Player class should have an abstract method named 
        level_up to be implemented in concrete classes.
        """
        pass
...
class Pawn(Player):
        ...
        and then set the moves attribute to a list of tuples representing x, y 
        coordinates.
        ...
        def __init__(self) -> None:
            super().__init__()
            self.moves : list[tuple[int, int]] =  [(1, 0), (0, 1), (-1, 0), (0, -1)]
        
        def level_up(self):
            """
            The Pawn class should implement a concrete level_up method by adding 
            more moves to the moves attribute. 
            The added moves should represent the four diagonal movements 
            (for example, 1 unit down plus 1 unit left).
            """
